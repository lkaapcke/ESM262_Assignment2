...
foo <- flights_db %>% select(year:day, dep_delay, arr_delay)
foo %>% show_query() # This shows you what SQL R used to create the tibble you requested

count_names <- gaz_db %>%
  group_by(FEATURE_NAME) %>% 
  summarize(n_names = n()) %>% 
  arrange(desc(n_names))
count_names %>% show_query()
