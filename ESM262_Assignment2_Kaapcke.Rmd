...
# Put new table in database
copy_to(gaz.db, class_types, "class_types",
  temporary = FALSE, 
  overwrite = TRUE
)

# Find the fractions of manmade vs. natural features for each county
feature_fractions <- dbGetQuery(gaz.db, "SELECT FEATURE_CLASS, count(FEATURE_CLASS) FROM class_types WHERE Class_Type ='Manmade' GROUP BY COUNTY_NAME;")
feature_fractions

# I had trouble getting past this point

# In order to get showquery to work, the code must be done on the table that exists in the database
# Copy HW2, put table in DB and run the same code
